 ...
 */
const RouterGurad = React.memo(function RouterGurad(props: { routes: RouteObject[] }) {
  const { routes } = props;

  const [searchParams] = useSearchParams();

  const [authorization, setAuthorization] = useLocalStorageState(config.JWT_LOCALSTORAGE_KEY); // 当前登录用户信息
  ...
  const [appLocale, setAppLocale] = useState<{ locale: string; message: any }>(); // app的国际化配置
